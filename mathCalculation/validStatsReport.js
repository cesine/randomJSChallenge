...
spc.xBar = function (arrayOfTrial) {
  ...
  let sum = 0;
  let totalLength = 0;
  for (let i = 0; i < arrayOfTrial.length; i++) {
    totalLength += arrayOfTrial[i].length;
    sum += arrayOfTrial[i].reduce((a, b) => a + b);
  }
  const rounded = sum / totalLength;
  return rounded;
};
...
spc.rBar = function (arrayOfTrial) {
  ...
  const rowByRowArray = [];
  // Go Row by Row.
  for (let row = 0; row < arrayOfTrial[0].length; row++) {
    const rowArray = [];
    for (let trial = 0; trial < arrayOfTrial.length; trial++) {
      // Goes from Row by Row on Trial 1,2,3
      // Restructuring to make it Row by row first.
      rowArray.push(arrayOfTrial[trial][row]);
    }
    const localMax = getMaxOfArray(rowArray);
    const localMin = getMinOfArray(rowArray);
    rowByRowArray.push(localMax - localMin);
  }

  return rowByRowArray.reduce((a, b) => a + b) / rowByRowArray.length;
};
...
spc.xBarMinMaxDiff = function (arrOfoperator) {
  ...
  const xbarArray = [];
  for (let operator = arrOfoperator.length - 1; operator >= 0; operator--) {
...
spc.rDoubleBarAvg = function (arrOfoperator) {
  ...
  return rbarArray.reduce((a, b) => a + b) / rbarArray.length;
};
...
spc.returnAllAnalysis = function (arrOfoperator, min, max, roundTo) {
  ...
  const rev = revHardcodedVal * spc.rDoubleBarAvg(arrOfoperator);
  const pcev = rev / (max - min);

  const rov = Math.sqrt(Math.pow((spc.xBarMinMaxDiff(arrOfoperator) * rovHardCodedVal), 2) - (Math.pow(rev, 2) / (nbrOfTrial * nbrOfpom)));
  const pcav = rov / (max - min);

  const rr = Math.sqrt(Math.pow(rev, 2) + Math.pow(rov, 2));
  const pcrr = Math.sqrt(Math.pow(pcev, 2) + Math.pow(pcav, 2));
...
cap.giveAvg = function (arrToCheck) {
  ...
  return arrToCheck.reduce((a, b) => a + b) / arrToCheck.length;
};
...
cap.zUpperSpec = function (target, maxTolerance, arrToCheck) {
  const avg = cap.giveAvg(arrToCheck);
  const stDev = cap.giveStDev(arrToCheck);
...
cap.findZQuality = function (arrToCheck, target, minTolerance, maxTolerance) {
  const ZUpper = cap.zUpperSpec(target, maxTolerance, arrToCheck);
  const Zlower = cap.zLowerSpec(target, minTolerance, arrToCheck);
  let minZValue = Zlower,
    PvalHi = 0,
    PvalLow = 0;
  ...
  const calcNormDistr = NormSInv(Math.min(Math.max(0.0000003, 1 - PvalHi - PvalLow), 0.9999996999996));
...
cap.giveMeThePassFailStatus = function (arrToCheck, target, minTolerance, maxTolerance, minZqual) {
  const zQual = cap.findZQuality(arrToCheck, target, minTolerance, maxTolerance);
