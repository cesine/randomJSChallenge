...
var all = require('./validStatsReport');
...
describe('It shoudl check all aspect of the Z test for the array', function() {
	var arrToCheck = [7.0180, 7.0200, 7.0150, 7.0190, 7.0160, 7.0160, 7.0200, 7.0220, 7.0150, 7.0160, 7.0180, 7.0220, 7.0260, 7.0210, 7.0160, 7.0150, 7.0160, 7.0250, 7.0240, 7.0160, 7.0230, 7.0180, 7.0160, 7.0160, 7.0140, 7.0200, 7.0210, 7.0230, 7.0200, 7.0160];
	var roundTo = 10000000,
		max = 0.036,
		min = 0.018,
		target = 7;
	it('should give me the average', function() {
		expect(Math.round(all.cap.giveAvg(arrToCheck) * roundTo) / roundTo).toEqual(7.0187667);
	});

	it('Should give me the StDev of the function', function() {
		expect(Math.round(all.cap.giveStDev(arrToCheck) * roundTo) / roundTo).toEqual(0.0033598);
	});

	it('Should give me the Upper Z value', function() {
		expect(Math.round(all.cap.zUpperSpec(7, 0.036, arrToCheck) * roundTo) / roundTo).toEqual(5.1292168);
	});

	it('Should give me the lower Z value', function() {
		expect(Math.round(all.cap.zLowerSpec(7, 0.018, arrToCheck) * roundTo) / roundTo).toEqual(10.9429907);
	});

	it('Should give me the Remaining of the Cumulative NormalDistValue of the Z Value', function() {
	...
	it('Should give me the Zst point', function() {
		expect(Math.round(all.cap.findZst(arrToCheck, min, max) * roundTo) / roundTo).toEqual(8.0361037);
	});

	it('Should give me the Z Quality', function() {
		expect(Math.round(all.cap.findZQuality(arrToCheck, target, min, max) * roundTo) / roundTo).toEqual(4.9912169);
	});

	it('Should give me the Final Pass-Fail result', function() {
		expect(all.cap.giveMeThePassFailStatus(arrToCheck, target, min, max, 4.5)).toBeTruthy();

		var arrToCheck2 = [21.0000, 21.9910, 21.9780, 21.9880, 21.9790, 21.9860, 21.9770, 21.9890, 21.9840, 21.9880, 21.9860, 21.9890, 21.9790, 21.9830, 21.9880, 21.9760, 21.9850, 21.9860, 21.9880, 21.9890, 21.9880, 21.9850, 21.9870, 21.9880, 21.9860, 21.9780, 21.9890, 21.9760, 21.9800, 21.9820];
